...
pub fn fwd_ctrl(
    ...
    if _mem_wb.ctr_unit.reg_write == 1
        && _mem_wb.rd != 0
        && _mem_wb.rd == _id_ex.rs
        && (_ex_mem.rd != _id_ex.rs || _ex_mem.ctr_unit.reg_write == 0)
    {
        fwd_unit.fwd_a = 1;
    } else if _ex_mem.ctr_unit.reg_write == 1 && _ex_mem.rd != 0 && _ex_mem.rd == _id_ex.rs {
        fwd_unit.fwd_a = 2;
    } else {
        fwd_unit.fwd_a = 0;
    }
    ...
    {
        fwd_unit.fwd_b = 1;
    } else if _ex_mem.ctr_unit.reg_write == 1 && _ex_mem.rd != 0 && _ex_mem.rd == _id_ex.rt {
        fwd_unit.fwd_b = 2;
    } else {
        fwd_unit.fwd_b = 0;
    }
