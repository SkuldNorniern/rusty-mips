...
pub fn fwd_ctrl(_ex_mem: &mut pipes::ExPipe,_mem_wb: &mut pipes::MemPipe, _id_ex: &mut pipes::IdPipe , _fwd_unit: forward_unit::FwdUnit) -> forward_unit::FwdUnit {
    let mut fwd_unit = _fwd_unit.clone();

    if _mem_wb.ctr_unit.mem_to_reg == 1 && _mem_wb.rd !=0 && _mem_wb.rd == _id_ex.rs && (_ex_mem.rd != _id_ex.rs || _ex_mem.ctr_unit.reg_write ==0 ){
        fwd_unit.fwd_a =1;
    }
    ...
    if _mem_wb.ctr_unit.mem_to_reg == 1 && _mem_wb.rd !=0 && _mem_wb.rd == _id_ex.rt && (_ex_mem.rd != _id_ex.rt || _ex_mem.ctr_unit.reg_write ==0 ){
        fwd_unit.fwd_b =1;
    }
